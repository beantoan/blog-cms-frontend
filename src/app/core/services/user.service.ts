...
export class UserService {
  ...
  populate() {
    ...
    if (this.jwtService.getToken()) {
      ...
      this.apiService.get<User>(ApiEndpoints.USER_ME)
        .subscribe(
  ...
  attemptAuth(credentials): Observable<AuthToken> {
    Logger.info(UserService.name, 'attemptAuth()', credentials);

    return this.apiService.post<AuthToken>(ApiEndpoints.USER_LOGIN, credentials)
      .pipe(
        take(1),
        map(data => {
